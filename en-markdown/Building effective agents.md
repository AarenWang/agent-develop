...
* **Agentic systems** — (Umbrella term used for both workflows and agents when distinguishing autonomy levels)
* **Workflows** — (Predefined LLM + tool orchestrations described as predictable options for well-scoped tasks)
* **Agents** — (LLM-driven systems that plan their own steps and tool usage with more flexibility)
* **Augmented LLM** — (Base building block that layers retrieval, tools, and memory on Claude models)
* **Retrieval** — (Used to ground model outputs with external knowledge before or during execution)
* **Tools / Tool Use block** — (API-exposed actions that Claude calls; responses include tool use blocks when invoked)
* **Memory (short- and long-term)** — (Scratchpad notes within a run and persistent stores for past sessions mentioned in agent loops)
* **Model Context Protocol (MCP)** — (Suggested approach for integrating third-party tools through a common client interface)
* **Agent-computer interface (ACI)** — (Guidance on crafting tool definitions so models can operate software reliably)
* **Prompt chaining** — (Workflow pattern that sequences multiple LLM calls with gating checks between steps)
* **Prompt tree-of-thought** — (Alternative branching workflow that explores multiple reasoning paths before selection)
* **Human-in-the-loop checkpoints** — (Review points used to bound agent autonomy when performing high-stakes actions)
* **Red-team testing** — (Automated stress tests recommended to probe agent behavior and safety before launch)
